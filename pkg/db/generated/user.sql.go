...
DELETE FROM users WHERE id = $1 AND NOT builtin
`
...
DELETE FROM users WHERE id = ANY($1::BIGINT[]) AND NOT builtin
RETURNING id
`
...
const setUserBuiltin = `-- name: SetUserBuiltin :exec
UPDATE users SET builtin = $1, updated_at = now()
WHERE id = $2
`

type SetUserBuiltinParams struct {
	Builtin bool  `json:"builtin"`
	ID      int64 `json:"id"`
}

func (q *Queries) SetUserBuiltin(ctx context.Context, arg SetUserBuiltinParams) error {
	_, err := q.db.Exec(ctx, setUserBuiltin, arg.Builtin, arg.ID)
	return err
}

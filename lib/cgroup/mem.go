...
func getMemLimitV2(sysfsPrefix, cgroupPath, statName string) (int64, error) {
	...
	for {
		...
		minLimit, err = updateMemLimitV2FromSubPath(sysfsPrefix, subPath, statName, minLimit)
		if err != nil {
			return 0, err
		}
		if subPath == "/" || subPath == "." {
...
// updateMemLimitV2FromSubPath reads statName under one sub path level and
// folds its value into minLimit (tracking the minimum positive limit). A
// missing file or "max" value leaves minLimit unchanged.
func updateMemLimitV2FromSubPath(sysfsPrefix, subPath, statName string, minLimit int64) (int64, error) {

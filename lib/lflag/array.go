...
func indexCloseQuote(s string, closeQuote byte) int {
	if closeQuote == '"' || closeQuote == '\'' {
		return indexCloseQuoteSimple(s, closeQuote)
	}
	return indexCloseQuoteBrace(s, closeQuote)
}

// indexCloseQuoteSimple handles the double/single-quote branch of
// indexCloseQuote: scan for the matching quote byte, skipping backslash-escaped
// occurrences. Extracted to keep indexCloseQuote a 2-line dispatch.
func indexCloseQuoteSimple(s string, closeQuote byte) int {
	idx := 0
	for {
...
// indexCloseQuoteBrace handles the brace branch of indexCloseQuote: scan for the
// matching close byte, recursing into nested quotes/braces. Extracted to keep
// indexCloseQuote a 2-line dispatch.
func indexCloseQuoteBrace(s string, closeQuote byte) int {
	idx := 0
	for {
		n := strings.IndexAny(s[idx:], `"'[{()}]`)
		if n < 0 {

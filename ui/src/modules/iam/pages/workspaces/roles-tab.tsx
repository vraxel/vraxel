...
export default function WorkspaceRolesTab() {
  ...
  const rolesQuery = useApiQuery({
    ...
    queryFn: () => {
      ...
      return listWorkspaceRoles(workspaceId, params)
    },
    placeholderData: keepPreviousData,
  })
  const roles = useMemo(() => rolesQuery.data?.items ?? [], [rolesQuery.data])
  const refresh = () => qc.invalidateQueries({ queryKey: ["iam", "workspace-roles", workspaceId] })
            ...
            {loading ? (
              ...
              roles.map((role) => (
                <TableRow key={role.metadata.id}>
                  ...
                  <TableCell>
                    <NameCell
                      to={`/iam/workspaces/${workspaceId}/roles/${role.metadata.id}`}
